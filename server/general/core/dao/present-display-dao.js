    ...
    presentDevice.findById(deviceId, function (err, foundDevice) {
        if (err) {
            ...
            if (foundDevice) {
                ...
                var error = new Error(consts.PRESENT_DEVICE_DOES_NOT_EXIST + deviceId);
                error.status = 422;
                callback(error, null);
            }
        }
...
function createLog(logObj, currentUser, finalCallback) {
	delete logObj._id;
    async.waterfall([
        function (callback) {
        ...
        if (createLogError) {
            finalCallback(createLogError, null);
        } else {
            finalCallback(null, savedLog);
        }
    });
}
