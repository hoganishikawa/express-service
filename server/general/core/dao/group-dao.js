    ...
    Group.findById(groupId, function (err, foundGroup) {
        if (err) {
            ...
            if (foundGroup) {
                ...
                var error = new Error(consts.GROUP_DOES_NOT_EXIST + groupId);
                error.status = 422;
                callback(error, null);
            }
        }
                                ...
                                async.waterfall([
                                    function (cCallback){
                                        if (children.sourceType === "Group") {
                                            ...
                                            tagDAO.getTagById(children.id, function (findErr, findResult) {
                                                if(findResult) {
                                                    childSources.push({
...
function createGroup(groupObj, currentUser, finalCallback) {
    ...
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        var error = new Error(consts.CAN_NOT_CREATE_GROUP);
        error.status = 422;
...
function editGroup(groupId, groupObj, currentUser, callback) {
    ...
    var error;
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        error = new Error(consts.CAN_NOT_EDIT_GROUP);
        error.status = 422;
        callback(error, null);
    } else {
        getGroupById(groupId, function (findErr, foundGroup) {
            if (findErr) {
                ...
                if (foundGroup.creatorRole === consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.BP) {
                    error = new Error(consts.ONLY_BP_CAN_SAVE_GROUP);
                    error.status = 422;
                    callback(error, null);
                } else {
...
function deleteGroupById(idArray, currentUser, finalCallback) {
    var error;
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        error = new Error(consts.CAN_NOT_DELETE_GROUP);
        error.status = 422;
        finalCallback(error, null);
    } else {
        async.waterfall([
            ...
            function (foundGroups, callback) {
                ...
                if(incorrect.length > 0) {
                    error = new Error(consts.ONLY_BP_CAN_DELETE_GROUP);
                    error.status = 422;
                    callback(error, null);
                } else {
                    callback(null, foundGroups);
