...
function createRule(ruleObj, currentUser, finalCallback) {
    ...
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        error = new Error(consts.CAN_NOT_CREATE_RULE);
        error.status = 422;
        ...
        async.waterfall([
                ...
                function (validateResult, callback) {
                    if (isReservedType(ruleObj)) {
                        error = new Error(consts.NOT_ALLOWED_RULE_TYPE);
                        error.status = 422;
                        callback(error, null);
                    } else {
                ...
                function (findRules, callback) {
                    ...
                    if (existingTypes.indexOf(ruleObj.tagType.toLowerCase()) > -1) {
                        error = new Error(consts.DUPLICATE_RULE_TYPE);
                        error.status = 422;
                        callback(error, null);
                    } else {
                        callback(null, ruleObj);
    ...
    TagRule.findById(selectedRuleId, function (err, findRule) {
        if (err) {
            ...
            if (findRule) {
                ...
                var error = new Error(consts.RULE_NOT_EXISTS + selectedRuleId);
                error.status = 422;
                callback(error, null);
            }
        }
...
function editRule(ruleId, ruleObj, currentUser, finalCallback) {
    ...
    var error;
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        error = new Error(consts.CAN_NOT_EDIT_RULE);
        error.status = 422;
        finalCallback(error, null);
    } else {
...
function deleteRuleById(selectedRuleId, currentUser, finalCallback) {
    ...
    var error;
    if (currentUser.role !== consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.Admin) {
        error = new Error(consts.CAN_NOT_DELETE_RULE);
        error.status = 422;
        finalCallback(error, null);
    } else {
        ...
        async.waterfall([
            ...
            function (findRule, callback) {
                if (findRule.creatorRole === consts.USER_ROLES.BP && currentUser.role !== consts.USER_ROLES.BP) {
                    error = new Error(consts.ONLY_BP_CAN_DELETE_RULE);
                    error.status = 422;
                    callback(error, null);
                } else {
                    callback(null, findRule);
            ...
            function(finRule, callback) {
                if(isReservedType(finRule)) {
                    error = new Error(consts.CAN_NOT_DELETE_RESERVED_RULE);
                    error.status = 422;
                    callback(error);
                } else {
