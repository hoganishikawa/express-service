        ...
        if(redisErr) {
            ...
            if(accountsStr) {
                ...
                if(findAccounts.length === 0) {
                    error = new Error(consts.INCORRECT_SF_ACCOUNT_ID);
                    error.status = 422;
                    callback(error, null);
                } else {
                    ...
                    if(findContacts.length === 0) {
                        error = new Error(consts.SF_CONTACT_ASSOCIATED_WITH_DIFFERENT_SF_ACCOUNT);
                        error.status = 422;
                        callback(error, null);
                    } else {
                ...
                error = new Error(consts.UNKNOWN_KEY);
                error.status = 422;
                callback(error, null);
            }
        }
...
function connecteBPUserTOSFDC(user, isSaveUser, callback) {
    if(user.role !== consts.USER_ROLES.BP) {
        var error = new Error(consts.REQUIRED_BP_ROLE);
        error.status = 422;
        callback(error);
    } else {
