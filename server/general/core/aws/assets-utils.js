        ...
        if (err) {
            ...
            for(var i = data.Contents.length - 1; i >= 0; i--) {
                ...
                imageData.sourceCDNURL = "https://" + consts.AWS_ASSETS_INFO.CNAMES[random.integer(1,997) % 3] + 
                    ".brightergy.com/" + data.Contents[i].Key;
                ...
                loadImagesFromAWS(keyPrefix, function(loadErr, resultImages){
                    if(loadErr) {
                        ...
                        if(resultImages && resultImages.length > 0) {
                            ...
                            var err = new Error(consts.ASSETS_NOT_LOADED);
                            err.status = 200;
                            callback(err, null);                
                        }
                    }
...
function uploadFile(parentFolderName, fileData, saveInCache, finalCallback) {
    ...
    if(!parentFolderName) {
        ...
        error = new Error(consts.UNKNOWN_FILE);
        error.status = 422;
        finalCallback(error, null);
    } else {
...
function deleteFile(keyPrefix, fileId, callback) {
    ...
    if(!keyPrefix) {
        ...
        error = new Error(consts.UNKNOWN_ASSETS_FILE_ID);
        error.status = 422;
        callback(error, null);
    } else {
        cache.get(ASSETS_KEY_PREFIX + keyPrefix, function(redisErr, imagesStr) {
            if(redisErr) {
                ...
                if(imagesStr) {
                    ...
                    if(image) {
                        ...
                        error = new Error(consts.NOT_FOUND_ASSETS_FILE + " " + fileId + " in " + keyPrefix);
                        error.status = 422;
                        callback(error);
                    }
                }
