...
router.post("/mobile", function(req, res, next) {
    ...
    var nonce = utils.getObjectValue(req.body, "nonce");

    if(!connectionToken || !nonce) {
        var error = new Error("Missing connection token or nonce");
        error.status = 422;
        return next(error);
    } else {
        socialLoginUtils.getUserBySocialLoginMobile(connectionToken, nonce, req, res, function(findErr, findUser) {
            if(findErr) {
                return next(findErr);
            } else {
                authUtils.loginUser(findUser, "iOS", req, res, next, function(loginErr, loginResult) {
                    if(loginErr) {
    ...
    if(!connectionToken) {
        ...
        socialLoginUtils.getUserBySocialLogin(connectionToken, req, res, function(findErr, findUser) {
            if(findErr) {
                ...
                authUtils.loginUser(findUser, null, req, res, next, function(loginErr, loginResult) {
                    if(loginErr) {
                        return next(loginErr);
                    } else {
...
router.get("/accounts/:userId", function(req, res, next) {
    ...
    userDAO.getUserById(userId, function(findUserErr, foundUser) {
        if (findUserErr) {
