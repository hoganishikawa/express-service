    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            if (currentUser.role === consts.USER_ROLES.BP) {
                ...
                var error = new Error(consts.CAN_NOT_UPLOAD_GENERAL_ASSETS);
                error.status = 422;
                return next(error);
            }
        }
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            if (currentUser.role === consts.USER_ROLES.BP) {
                ...
                var error = new Error(consts.CAN_NOT_DELETE_GENERAL_ASSETS);
                error.status = 422;
                return next(error);
            }
        }
...
router.post("/presentation/:objectId", function(req, res, next) {
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            presentationDAO.getPresentationById(objectId, currentUser, function(findErr, findObject) {
                if(findErr) {
                    return next(findErr);
                } else {
...
router.post("/dashboard/:dashboardId", function(req, res, next) {
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            dashboardDAO.getDashboardById(dashboardId, currentUser, function(findErr, findObject) {
                if(findErr) {
                    return next(findErr);
                } else {
                    if(findObject.awsAssetsKeyPrefix) {
