 ...
 */
router.post("/newutilityprovider", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            if (!text) {
                var error = new Error(consts.UNKNOWN_UTILITY_PROVIDER);
                error.status = 422;
                return next(error);
            } else {

                accountDAO.getAccountsByUser(currentUser, null, null, function (err, findAccounts) {
                    if (err) {
                        return next(err);
                    } else {
                        ...
                        emailSender.sendNewUtilityProviderEmail(accountsUrls, currentUser.sfdcContactURL, text, 
 ...
 * @apiError message Error code
 */
router.post("/presentationlink", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            ...
            if (!message) {
                validateErrors.push(consts.MESSAGE_REQUIRED);
            }
            if (!link) {
                validateErrors.push(consts.PRESENTATION_LINK_REQUIRED);
            }
            if (!subject) {
 ...
 * @apiError message Error code
 */
router.post("/dashboardlink", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            ...
            if (!link) {
                validateErrors.push(consts.DASHBOARD_LINK_REQUIRED);
            }
            if (!subject) {
