...
function startLoadingData(cachedData, presentation, widget, widgetType, tempoIQItems, socket, finalCallback) {
    if (widgetType === consts.BRIGHTERVIEW_WIDGET_TYPES.Solar) {
        ...
        forecastWrapper.getWeatherWidgetData(widget, presentation, tempoIQItems, socket, finalCallback);
...
function runWidgetCaclulator(presentation, widget, widgetType, tempoIQItems, socket, finalCallback) {
    if(socket) {
        ...
        cacheUtils.getWidgetData(socket.id, presentationId, widgetId, function(cacheErr, loadedObj) {
            if(cacheErr) {
        ...
        if (widgetType === consts.BRIGHTERVIEW_WIDGET_TYPES.Weather) {
            ...
            if (!dateRange) {
                ...
                if (dateRange === "-- Custom --") {
                    ...
                    endDate = moment.utc().millisecond(0).add("h", -6);
                    //log.info('endDate1 %s', endDate.toISOString());
                    if (dateRange === "Day") {
                        startDate = endDate.clone().add("d", -1);
                    } else if (dateRange === "3 Days") {
                        startDate = endDate.clone().add("d", -2);
                    } else if (dateRange === "Week") {
                        startDate = endDate.clone().add("w", -1);
                    } else if (dateRange === "Month") {
                        startDate = endDate.clone().add("months", -1);
                    } else if (dateRange === "Year") {
                        startDate = endDate.clone().add("years", -1);
                    } else if (dateRange === "All") {
                        startDate = null;
                        endDate = null;
                    } else {
