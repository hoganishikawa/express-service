...
function calculateEnergyData(cachedData, presentationDTO, tempoIQQueryParams, socket, finalCallback) {
    ...
    async.map(tempoIQQueryParams, function(tempoIQParam, callback) {
        ...
        if(loadAllExistingData) {
            tempoiq.loadAllExistingData(selection, null, tempoIQParam, function (err, result) {
            ...
            tempoiq.loadData(tempoIQParam.startDate, tempoIQParam.endDate, selection, null, tempoIQParam,
                function(err, result) {
                    processTempoIQItem(err, totalTempoIQResults, result, presentationDTO, socket, callback);
                });
        }
        ...
        if(err) {
            ...
            if(socket) {
                ...
                cacheUtils.setWidgetData(socket.id, presentationId,
                    presentationId, totalTempoIQResults, function(cacheErr, cacheResult) {
