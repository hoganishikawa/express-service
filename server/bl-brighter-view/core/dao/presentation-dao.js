...
function getPresentationById(selectedPresentationId, user, finalCallback) {
    if(user && !permissionsUtils.userHaveAccessToPresent(user)) {
        var error = new Error(consts.NOT_ACCESSIBLE_PRESENT_APP);
        error.status = 422;
        finalCallback(error);
    } else {
        async.waterfall([
            ...
            function(findPresentation, callback) {
                if(!findPresentation) {
                    var error = new Error(consts.PRESENTATION_NOT_EXISTS + selectedPresentationId);
                    error.status = 422;
                    callback(error);
                } else {
            ...
            function(findPresentation, accessResult, callback) {
                if(accessResult) {
                    ...
                    var error = new Error(consts.PRESENTATION_NOT_ACCESSIBLE);
                    error.status = 422;
                    callback(error);
                }
            }
            ...
            if (findErr) {
                ...
                findPresentation.save(function (saveErr, savedPresentation) {
                    if (saveErr) {
                        ...
                        cacheUtils.deleteSingleAppEntityCache(savedPresentation._id.toString(),
        ...
        .exec(function (err, findPresentation) {
            if(err) {
                ...
                if(findPresentation) {
                    ...
                    var error = new Error(consts.TEMPLATE_NOT_EXISTS);
                    error.status = 422;
                    callback(error, null);
                }
            }
    ...
    getPresentationById(selectedPresentationId, currentUser, function(findErr, findPresentation) {
        if(findErr) {
            ...
            if(bpRoleCheck || adminRoleCheck || tmRoleCheck) {
                var error = new Error(consts.CAN_NOT_DELETE_PRESENTATION);
                error.status = 422;
                callback(error, null);
            } else {
                removePresentationFromAllTags(findPresentation, function(err, result) {
                    if(err) {
                        ...
                        cacheUtils.deleteSingleAppEntityCache(findPresentation._id.toString(),
                            function(cacheErr, cacheResult) {
                            if(cacheErr) {
                                callback(cacheErr);
                            } else {
...
function addEditor(presentationId, userId, currentUser, finalCallback) {
    if (currentUser.role !== consts.USER_ROLES.BP) {
        var error = new Error(consts.ONLY_BP_CAN_ADD_PRESENTATION_EDITOR);
        error.status = 422;
...
function removeEditor(presentationId, userId, currentUser, finalCallback) {
    if (currentUser.role !== consts.USER_ROLES.BP) {
        var error = new Error(consts.ONLY_BP_CAN_REMOVE_PRESENTATION_EDITOR);
        error.status = 422;
        finalCallback(error, null);
    }
...
function getPresentationTagsFullHierarchy(findPresentation, finalCallback) {
    if(findPresentation.tagBindings && findPresentation.tagBindings.length>0) {
        ...
        var error = new Error(consts.PRESENTATION_TAGS_NOT_REGISTERED);
        error.status = 422;
        finalCallback(error);
    }
}
