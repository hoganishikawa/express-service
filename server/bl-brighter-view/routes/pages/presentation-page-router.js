...
router.get("/", function(req, res, next) {
    ...
    if(!presentationId) {
        res.render("presentation", {errors: consts.MISSING_PRESENTATION_ID,
            presentation: null, loadWidgets:null, config: viewConfig });
    } else if(!isValidId) {
        res.render("presentation", {errors: consts.INCORRECT_PRESENTATION_ID,
            presentation: null, loadWidgets:null, config: viewConfig });
    } else {
        ...
        async.parallel([
            ...
            if (err) {
                res.render("presentation", {errors: err.message, presentation: null, loadWidgets:null,
                    config: viewConfig });
            } else {
