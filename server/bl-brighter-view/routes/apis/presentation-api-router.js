...
router.post("/", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            if(!params.name) {
                var error = new Error(consts.PRESENTATION_NAME_REQUIRED);
                error.status = 422;
                return next(error);
            } else {

                presentationDAO.createPresentation(currentUser, params.name, params.templateId, params.bpLock,
                    function (err, result) {
                    if (err) {
                        return next(err);
                    } else {
 ...
 * @apiError message Error code
 */
router.put("/:presentationId", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            ...
            presentationDAO.validate(presentationObj, function(validateErr, status) {
                if(validateErr) {
                    ...
                    presentationDAO.savePresentation(currentUser, presentationObj, false, function (err, result) {
                        if (err) {
                            return next(err);
                        } else {
                            return utils.successResponse(result, res, next);
...
router.get("/", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            presentationDAO.getPresentationsByUser(currentUser, function (err, findPresentations) {
                if (err) {
                    return next(err);
                } else {
 ...
 * @apiError message Error code
 */
 router.get("/:presentationId/editors", function(req, res, next){
    var presentationId = req.params.presentationId;
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if(findUserErr) {
            ...
            presentationDAO.getEditors(presentationId, currentUser, function (err, editors) {
                if (err) {
                    return next(err);
                } else {
 ...
 */
router.post("/:presentationId/editors", function(req, res, next){
    var params = req.body;
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if(findUserErr) {
            ...
            presentationDAO.addEditor(presentationId, userId, currentUser, function (err, editors) {
                if (err) {
                    return next(err);
                } else {
                    return utils.successResponse(editors, res, next);
 ...
 * @response    Array of User Object or error
 */
router.delete("/:presentationId/editors", function(req, res, next){
    var params = req.body;
    var userId = params.userId;
    var presentationId = params.presentationId;
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if(findUserErr) {
            ...
            presentationDAO.removeEditor(presentationId, userId, currentUser, function (err, editors) {
                if (err) {
                    return next(err);
                } else {
                    return utils.successResponse(editors, res, next);
 ...
 * @apiError message Error code
 */
router.post("/clone/:presentationId", function(req, res, next) {
    ...
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            presentationDAO.clone(presentationId, currentUser, function(err, clonedPresentation) {
                if(err) {
                    return next(err);
                } else {
                    currentUser.previousEditedPresentation = presentationId;
                    currentUser.lastEditedPresentation = clonedPresentation._id;

                    currentUser.save(function (saveErr, savedUser) {
                        if(saveErr) {
                            return next(saveErr);
                        } else {
 ...
 * @apiError message Error code
 */
router.delete("/:presentationId", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            var presentationId = req.params.presentationId;
            presentationDAO.deletePresentationById(presentationId, currentUser, function (err, answer) {
                if (err) {
                    return next(err);
                } else {
 ...
 * @apiError message Error code
 */
router.post("/:presentationId/tags", function(req, res, next) {
    var presentationId = req.params.presentationId;
        ...
        if (findUserErr) {
            ...
            presentationDAO.getPresentationById(presentationId, currentUser, function (findErr, foundPresentation) {
                if (findErr) {
                    ...
                    presentationDAO.addTag(currentUser, foundPresentation, tagObj, function (error, savedPresentation) {
                        if (error) {
                            return next(error);
                        } else {
 ...
 */
router.put("/:presentationId/tags", function(req, res, next) {
    var presentationId = req.params.presentationId;
    ...
    checkAuth(req, res, next, function (findUserErr, currentUser) {
        if (findUserErr) {
            ...
            presentationDAO.getPresentationById(presentationId, currentUser,
                function (findErr, foundPresentation) {
                if (findErr) {
                    return next(findErr);
                } else {
                    if(foundPresentation.tagBindings && foundPresentation.tagBindings.length > 0) {
                        presentationDAO.removeTagsAll(currentUser, foundPresentation,
                            function (error, removedPresentation) {
                            if (error) {
                                return next(error);
                            } else {
                                presentationDAO.addTags(currentUser, removedPresentation, tagBindings,
                        ...
                        presentationDAO.addTags(currentUser, foundPresentation, tagBindings,
                            function (error, savedPresentation) {
                            if (error) {
                                return next(error);
                            } else {
 ...
 * @apiError message Error code
 */
router.delete("/:presentationId/tags/:tagId", function(req, res, next) {
    var presentationId = req.params.presentationId;
        ...
        if (findUserErr) {
            ...
            presentationDAO.getPresentationById(presentationId, currentUser, function (findErr, foundPresentation) {
                if (findErr) {
                    ...
                    tagDAO.getTagById(tagId, currentUser, function (findTagErr, foundTag) {
                        if(findTagErr) {
                            return next(findTagErr);
                        } else {
                            presentationDAO.removeTag(currentUser, foundPresentation, tagId,
                                function (error, savedPresentation) {
                                if (error) {
                                    return next(error);
                                } else {
...
router.post("/widgets", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            ...
            widgetDAO.createWidget(widgetObj, currentUser, function (err, result) {
                if (err) {
                    return next(err);
                } else {
                    return utils.successResponse(result, res, next);
 ...
 * @apiError message Error code
 */
router.put("/widgets/:widgetId", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            ...
            widgetDAO.updateWidget(widgetObj, widgetId, currentUser, function (err, result) {
                if (err) {
                    return next(err);
                } else {
                    return utils.successResponse(result, res, next);
 ...
 * @apiError message Error code
 */
router.delete("/widgets/:widgetId", function(req, res, next) {
    checkAuth(req, res, next, function(findUserErr, currentUser) {
        if (findUserErr) {
            return next(findUserErr);
        } else {
            var widgetId = req.params.widgetId;
            widgetDAO.deleteWidgetById(widgetId, currentUser, function (err, answer) {
                if (err) {
                    return next(err);
                } else {
                    return utils.successResponse(answer, res, next);
