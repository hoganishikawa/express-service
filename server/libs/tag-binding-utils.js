            ...
            async.each(uniqueTagBindingsArray, function (tagBindingPair, tagBindingsCallback) {
                async.waterfall([
                    function (callback) {
                        if (consts.APP_ENTITY_TYPES.indexOf(objectType) > -1) {
                            Tag.findOne(
                                {$and: [{_id: new ObjectId(tagBindingPair.id)}, {appEntities: objectBindPair}]},
                                function (error, foundTag) {
                                    callback(error, foundTag);
                                });
                        } else if (consts.USER_WITH_ACCESS_TYPES.indexOf(objectType) > -1) {
                            Tag.findOne(
                                {$and: [{_id: new ObjectId(tagBindingPair.id)}, {usersWithAccess: objectBindPair}]},
                                function (error, foundTag) {
                                    callback(error, foundTag);
                                });
                        } else {
                            callback(consts.INVALID_ENTITY_TYPE, null);
                        }
                    },
                    function (isEntityBoundToTagAlready, callback) {
                        if (isEntityBoundToTagAlready) {
                            ...
                            Tag.update({_id: new ObjectId(tagBindingPair.id)},
                                {$push: objectBindPairQuery}, callback);
                        }
                    }
                ], function (updateError, updatedTag) {
                    if (updateError) {
                        tagBindingsCallback(updateError, null);
                    } else {
                        tagBindingsCallback(null, updatedTag);
                    }
                });
    ...
    async.waterfall([
        ...
        function(safeToUnbindTagBindingsArray, callback) {
            ...
            if (objectBindPairQuery === {}) {
                finalCallback(new Error(consts.ERROR_IN_OBJECT_BINDING_PAIR), null);
            }
            
            if (safeToUnbindTagBindingsArray) {
                async.eachSeries(safeToUnbindTagBindingsArray, function (tagBindingId, unbindingsCallback) {
                        Tag.update({_id: new ObjectId(tagBindingId)},
                            { $pull: objectBindPairQuery }, unbindingsCallback);
                    },
