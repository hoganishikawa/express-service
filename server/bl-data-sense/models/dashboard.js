...
module.exports = function() {
    ...
    dashboardSchema.pre("save", function (next) {
        ...
        if (utils.isCreratorRoleChanged(obj)) {
            ...
            error = new Error(consts.NOT_UNIQUE_COLLECTIONS);
            error.status = 422;
            return next(error);
        } else if(obj.startDate && obj.endDate && obj.startDate > obj.endDate) {
            error = new Error(consts.INCORRECT_DATA_RANGE);
            error.status = 422;
            return next(error);
            ...
            error = new Error(consts.NOT_ALLOWED_SUB_DAY);
            error.status = 422;
            return next(error);
        } else {
            ...
            if(primaryDateRangeChanged || compareDateRangeChanged) {
                cacheUtils.deleteSingleAppEntityCache(obj._id.toString(), function(err, res) {
                    if(err) {
                        return next(err);
                    } else {
                        return next();
