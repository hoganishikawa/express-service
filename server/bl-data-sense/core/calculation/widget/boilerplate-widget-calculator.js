...
BoilerplateWidgetCalculator.prototype.getInformations = function(widget, retCallback) {
    ...
    if (!geoDataItem.latitude || !geoDataItem.longitude) {
        retCallback(consts.GEO_NOT_EXIST, null);
    } else {
        forecastWrapper.loadCurrentForecastData(geoDataItem, function(err, result){
            if (err) {
                retCallback(err, null);
            } else {
                returnObj.Temperature = result.temperature;
                returnObj.WeatherStatus = result.summary;
