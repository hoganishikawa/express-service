        ...
        Widget.findById(selectedWidgetId)
            ...
            if(err) {
                ...
                if(findWidget) {
                    ...
                    var error = new Error(consts.WIDGET_NOT_EXISTS + selectedWidgetId);
                    error.status = 422;
                    callback(error, null);
                }
            }
...
function deleteWidgetById(dashboardId, selectedWidgetId, currentUser, finalCallback) {
    async.waterfall([
        ...
        function (findDashboard, callback) {
            ...
            if(index < 0) {
                var error = new Error(consts.WIDGET_NOT_ADDED_TO_DASHBOARD);
                error.status = 422;
                callback(error);
            } else {
                ...
                if(findWidgetErr) {
                    ...
                    Widget.remove({_id: findWidget._id}).exec(function(deleteErr, deleteResult) {
                        if(deleteErr) {
                            ...
                            cacheUtils.deleteAppEntityWidgetCache(dashId, widgetId, callback);
                        }
                    });
                }
            });
    ...
    if(!permissionsUtils.userHaveAccessToAnalyze(user)) {
        ...
        async.waterfall([
            ...
            function(savedWidget, affected, cb) {
                cacheUtils.deleteAppEntityWidgetCache(dashboardId, savedWidget._id.toString(),
...
function saveWidgetAndDeleteCache(dashboard, widget, callback) {
    ...
    widget.save(function(err, savedWidget) {
        if(err) {
            ...
            cacheUtils.deleteAppEntityWidgetCache(dashboard._id.toString(), savedWidget._id.toString(),
                function(cacheErr, cacheResult) {
                if(cacheErr) {
                    callback(cacheErr);
                } else {
