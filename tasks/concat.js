...
module.exports = function(gulp) {
    ...
    var blComponents = {
        'platformpanel': {
            src: ['client/components/platform-panel/platform-panel.js',
                'client/components/platform-panel/js/services/**/*.js',
                'client/components/platform-panel/js/directives/**/*.js',
                'client/components/platform-panel/js/controllers/**/*.js',
                'client/components/platform-panel/templates/dist/**/*.js'
                ],
            dest: 'client/components/platform-panel/dist'
        }
    };
    ...
    function getConcatStreamByComponent(component) {
        return gulp.src(blComponents[component].src)
            .pipe(plumber())
            .pipe(concat(component + '.js'))
            .pipe(stripDebug())
            .pipe(uglify({mangle: false}))
            .pipe(rename('component-' + component + '.min.js'))
            //.pipe(cachebust.resources())
            .pipe(gulp.dest(blComponents[component].dest));
    }
    ...
    for (var component in blComponents) {
        if (blComponents.hasOwnProperty(component)) {
            (function (componentName) {
                gulp.task('build-component-' + componentName, function () {
                    return getConcatStreamByComponent(componentName);
                });
            })(component);
        }   
    }
